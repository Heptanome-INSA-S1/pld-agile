...
class RoundModifierImp(
    ...
    override fun addDelivery(delivery: Delivery, round: Round) {
        if (isDeliveryValid(delivery)) {
            ...
            val listEarliestEndTime = getEarliestEndTime(round)
            val listLatestEndTime = getLastestEndTime(round)
            ...
            round.intersections()
                   ...
                   /*     val nextDuration = (round.deliveries().firstOrNull { it.address == round.intersections()[i+1] }?.duration) ?: 0.seconds
                        val earliestArrivalTime = listEarliestEndTime.get(0) + prevShortestPathDuration
                        var earliestDepartureTime = if(delivery.startTime != null && delivery.startTime > earliestArrivalTime) delivery.startTime else earliestArrivalTime
                        earliestDepartureTime += delivery.duration
                        if ((getNextNonNull(listLatestEndTime, i) != null && prevShortestPathDuration.length + delivery.duration.toSeconds() + nextShortestPathDuration.length + nextDuration.toSeconds() < (getNextNonNull(listLatestEndTime, i)!!.toSeconds() - listEarliestEndTime[i]!!.toSeconds()))
                                && (delivery.endTime == null || (delivery.endTime != null && delivery.endTime!! > earliestDepartureTime))
                                && (delivery.startTime == null || (delivery.startTime != null && delivery.startTime!! > listEarliestEndTime.get(i) + prevShortestPathDuration))) {
                            listSubPath.add(i, SubPath(prevShortestPath, prevShortestPathDuration, delivery, nextShortestPath, nextShortestPathDuration))
                        } else if(getNextNonNull(listLatestEndTime, i) == null) {
                            listSubPath.add(i, SubPath(prevShortestPath, prevShortestPathDuration, delivery, nextShortestPath, nextShortestPathDuration))
                        }else {
                            listSubPath.add(i, null)
                        }*/
